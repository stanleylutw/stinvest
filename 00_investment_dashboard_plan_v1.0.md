...
| UI-6 | X 軸日期標籤在特定資料點數量下，右側兩個標籤距離太近重疊成亂碼；Y2 開啟時座標軸數字未貼齊右邊界，留下死空間，圖表右側留白過多 | `index.html` `renderHistoryTrendChart` | ✅ 已修復：日期標籤新增防碰撞判斷；Y2 座標軸數字改貼齊右邊界並縮小 margin（branch `fix_history_chart_label_gap`）。已知極窄邊界案例（Minor，見 `docs/REVIEW_REPORT.md`）尚未處理 |
